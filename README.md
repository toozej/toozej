### Hi there 👋

I'm toozej: a DevOps & Infrastructure engineer, audiophile, mountain fiend, photographer, car nerd, ~loud typist~ [mech keyboard enthusiast](https://github.com/toozej/keebs), FOSS-ist, skier, PNW-er. Once in IT, always in IT.

#### 👨💻 Repositories I created recently

- [lego-stego](https://github.com/toozej/lego-stego) - Go-based CLI tool for Steganography
- [go-find-archived-gh-actions](https://github.com/toozej/go-find-archived-gh-actions) - Find archived GitHub Actions in use in GHA Workflows with optional notifications and GH Issue creation
- [tools](https://github.com/toozej/tools) - Tools and web-apps
- [terranotate](https://github.com/toozej/terranotate) - Terraform Comment Parser and Validator
- [gcp-terraform-wrapper-example](https://github.com/toozej/gcp-terraform-wrapper-example) - Example scaffolding for building opinionated wrapper modules around official terraform-google-modules

#### ⛏️ What I've been working on

- [rss2socials](https://github.com/toozej/rss2socials) - Watches a RSS feed for new posts, then announces them on Mastodon (1 day ago)
- [python-starter](https://github.com/toozej/python-starter) - python-starter (1 day ago)
- [go-find-archived-gh-actions](https://github.com/toozej/go-find-archived-gh-actions) - Find archived GitHub Actions in use in GHA Workflows with optional notifications and GH Issue creation (1 day ago)
- [EXIFizer](https://github.com/toozej/EXIFizer) - Bulk EXIF Tagger for Film Photography (1 day ago)
- [go-find-liquor](https://github.com/toozej/go-find-liquor) - Go Find Liquor (in Oregon) (2 days ago)
- [kmhd2spotify](https://github.com/toozej/kmhd2spotify) - Automatically sync songs from KMHD jazz radio to your Spotify playlists (2 days ago)
- [lego-stego](https://github.com/toozej/lego-stego) - Go-based CLI tool for Steganography (2 days ago)
- [golang-starter](https://github.com/toozej/golang-starter) - Golang starter template (2 days ago)
- [photos2map](https://github.com/toozej/photos2map) - Generate GPX map from photos' EXIF data (2 days ago)
- [trails-completionist](https://github.com/toozej/trails-completionist) - Golang application to parse a list of trails, then display a searchable HTML table for ease of tracking completion of trails. (2 days ago)
